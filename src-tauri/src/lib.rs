...
async fn list_containers(state: State<'_, AppState>) -> Result<Vec<Container>, CommandError> {
    ...
    let containers_vec: Vec<Container> = containers
        .into_iter()
        .map(|item| Container {
...
async fn emit_logs(
    ...
    while let Some(log_result) = logs_stream.next().await {
        match log_result {
            Ok(log) => on_event
                .send(log.to_string())
                .map_err(|e| CommandError::UnexpectedError(format!("Failed to send log: {}", e)))?,
            Err(e) => {
...
async fn remove_image(state: State<'_, AppState>, image: &str) -> Result<(), CommandError> {
    ...
    docker
        ...
        .await
        .map_err(|e| {
            CommandError::DockerError(format!("Failed to remove image: {} : {}", image, e))
        })?;
    Ok(())
}
